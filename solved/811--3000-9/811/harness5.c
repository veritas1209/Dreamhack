...
int main(int argc,char**argv){
    ...
    // position -> name table and content_by_pos
    ...
    // extract op4 sbox (byte-wise, ctx-independent) and build inverse
    unsigned char S[256],Sinv[256];
    for(int v=0;v<256;v++){ unsigned char b[16]={0}; b[0]=(unsigned char)v; memcpy((char*)CTX,K[0],16); OP(CTX,b,4); S[v]=b[0]; }
    for(int v=0;v<256;v++) Sinv[S[v]]=(unsigned char)v;
    printf("S[0]=%02x S[1]=%02x (expect 63 7c)\n",S[0],S[1]);

    #define OP6INV(b) do{ memcpy((char*)CTX,K[0],16); OP(CTX,(b),6); OP(CTX,(b),6); OP(CTX,(b),6);}while(0)
    #define OP8INV(b) do{ memcpy((char*)CTX,K[0],16); for(int _i=0;_i<7;_i++) OP(CTX,(b),8);}while(0)
    ...
    for(int p=0;p<19;p++){
        ...
        for(int r=16;r>=1;r--){ int m=p*16+(r-1);
            if(r<16) OP8INV(blk);
            OP6INV(blk);
            for(int i=0;i<16;i++) blk[i]=Sinv[blk[i]];   // op4 inverse (InvSubBytes)
            for(int i=0;i<16;i++) blk[i]^=K[m][i];        // AddRoundKey
        }
        memcpy(PT[p],blk,16);
        ...
        printf("pos %2d PT=%s |%s| %s\n",p,ph,a,ok?"MATCH":"NOMATCH");
    }
    printf("=== ASSEMBLED PATH ===\n");
    for(int p=0;p<19;p++){ if(!havep[p])continue; for(int i=0;i<16;i++){ unsigned char ch=PT[p][i]; putchar(ch?ch:'.'); } }
    printf("\n");
    printf("=== END ===\n"); return 0;
}
