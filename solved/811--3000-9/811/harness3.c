...
int main(int argc,char**argv){
    ...
    // position -> name table (key state) and content_by_pos
    char names[40][64]; for(int p=0;p<40;p++) nameAt(p,names[p]);
    char ctbypos[19][33]; int havep[19]; for(int i=0;i<19;i++)havep[i]=0;
    for(int c=0;c<NCH;c++){ int pos=-1; for(int p=0;p<40;p++) if(!strcmp(strip0(names[p]),strip0(CH[c][0]))){pos=p;break;}
        if(pos>=0&&pos<19){ strcpy(ctbypos[pos],CH[c][1]); havep[pos]=1; } }

    // ---- key schedule K[m] = ctx after m op10 updates ----
    static unsigned char K[320][16];
    if(setjmp(JB)){ printf("[keysched exception]\n"); return 0; }
    initCtx(); memcpy(K[0],CTX,16);
    for(int m=1;m<320;m++){ OP(CTX,CTX,10); memcpy(K[m],CTX,16); }
    { // verify K[16] matches name(0)
      unsigned long long q8=*(unsigned long long*)(K[16]+8),q0=*(unsigned long long*)(K[16]+0);
      char nm[64]; sprintf(nm,"%llx%llx",q8,q0);
      printf("Kcheck name(0)=%s expect=5dfb005def07e285c215b963a99b039b %s\n",nm,!strcmp(nm,"5dfb005def07e285c215b963a99b039b")?"OK":"BAD");
    }

    // ---- op involution test at K[0] ----
    printf("=== OP INVOLUTION TEST (at K[0]) ===\n");
    for(int t=0;t<3;t++){ int tag=(int[]){4,6,8}[t];
        unsigned char b[16],b1[16]; for(int i=0;i<16;i++)b[i]=(unsigned char)(0x11*i+7);
        memcpy(b1,b,16);
        setjmp(JB);
        memcpy((char*)CTX,K[0],16); OP(CTX,b1,tag);
        memcpy((char*)CTX,K[0],16); OP(CTX,b1,tag);
        printf("op%d involution: %s\n",tag,!memcmp(b,b1,16)?"YES":"no");
    }

    // ---- inverse-round decrypt (assume op involutions) + re-encrypt verify ----
    printf("=== DECRYPT + VERIFY ===\n");
    static unsigned char PT[19][16];
    for(int p=0;p<19;p++){
        if(!havep[p]){ printf("pos %d: missing\n",p); continue; }
        unsigned char blk[16]; hex2b(ctbypos[p],blk,16);
        if(setjmp(JB)){ printf("pos %d: exception during decrypt\n",p); continue; }
        for(int r=16;r>=1;r--){ int m=p*16+(r-1);
            memcpy((char*)CTX,K[m],16);
            if(r<16) OP(CTX,blk,8);
            OP(CTX,blk,6);
            OP(CTX,blk,4);
            for(int i=0;i<16;i++) blk[i]^=K[m][i];
        }
        memcpy(PT[p],blk,16);
        // re-encrypt verify
        unsigned char chk[16]; memcpy(chk,blk,16);
        initCtx(); for(int k=0;k<p;k++){unsigned char j[16];memset(j,0,16);ENC(CTX,j);} ENC(CTX,chk);
        char ph[33]; b2hex(blk,16,ph); char ch[33]; b2hex(chk,16,ch);
        int ok=!strcmp(ch,ctbypos[p]);
        char a[17]; for(int i=0;i<16;i++)a[i]=(blk[i]>=32&&blk[i]<127)?blk[i]:'.'; a[16]=0;
        printf("pos %2d PT=%s |%s| verify=%s\n",p,ph,a,ok?"MATCH":"NOMATCH");
    }
    // assemble
    printf("=== ASSEMBLED PATH ===\n");
    for(int p=0;p<19;p++){ if(!havep[p])continue; for(int i=0;i<16;i++){ unsigned char ch=PT[p][i]; putchar((ch>=32&&ch<127)?ch:'.'); } }
    printf("\n");
    printf("=== END ===\n"); return 0;
}
