...
int main(int argc,char**argv){
    ...
    ENC=(enc_t)(gbase+0x1fb0); CTX=(void*)(gbase+0xd9e8);
    ...
    char names[MAXP][64]; for(int p=0;p<MAXP;p++) nameAt(p,names[p]);
    printf("=== DECRYPT ATTEMPT (involution) ===\n");
    for(int c=0;c<NCH;c++){
        int pos=-1; for(int p=0;p<MAXP;p++) if(!strcmp(strip0(names[p]),strip0(CH[c][0]))){pos=p;break;}
        unsigned char ct[16],out[16]; hex2b(CH[c][1],ct,16);
        if(pos>=0 && encAt(pos,ct,out)){ char oh[33]; b2hex(out,16,oh); char a[17]; for(int i=0;i<16;i++)a[i]=(out[i]>=32&&out[i]<127)?out[i]:'.'; a[16]=0;
            printf("name=%s pos=%2d E_p(ct)=%s |%s|\n",CH[c][0],pos,oh,a);
        } else printf("name=%s pos=%2d (nomatch)\n",CH[c][0],pos);
    }
    printf("=== END ===\n"); return 0;
}
