...
class TinyGHOST:
    ...
    def _expand_key(self, key: bytes) -> list[list[int]]:
        ...
        for i in range(0, 32, 16):
            round_keys.append(key_bits[i:i+16])
        return round_keys

    def _substitution(self, bit16: list[int]) -> list[int]:
        res = []
        for i,j in enumerate(range(0,16,4)):
            res += int_to_bits(self.sbox[3-i][bits_to_int(bit16[j:j+4])], 4)
        return res

    def _round_function(self, round_n: int, is_enc: bool) -> None:
        if round_n > 15:
            round_key_idx = (round_n+1)%2
        else:
            round_key_idx = round_n%2

        state_hi = self._state[:16]
        state_lo = self._state[16:]

        state_lo = add_mod_2_16(state_lo, self._round_keys[round_key_idx])
        state_lo = self._substitution(state_lo)
        state_lo = rol11(state_lo)
        state_lo = xor_lst(state_lo, state_hi)

        if (is_enc and round_n == 29) or (not is_enc and round_n == 0):
            self._state[:16] = state_lo
        else:
            self._state[:16] = self._state[16:]
            self._state[16:] = state_lo
