...
class Misty1:
    ...
    def decrypt_block(self, src: bytes) -> bytes:
        ...
        return put_uint32(d0) + put_uint32(d1)


# 브루트포스 공격
import time

ciphertext = bytes.fromhex("792c6cfd902bbed9")
total_keys = 16 * 256 * 256

print("=" * 70)
print("MISTY1 Brute Force Attack - Decryption Method")
print("=" * 70)
print(f"Target ciphertext: {ciphertext.hex()}")
print(f"Total key space: {total_keys:,} keys")
print(f"Strategy: Decrypt with each key and check if result starts with 'DH{{'")
print("=" * 70)
print()
...
for b14 in range(0xf0, 0x100):
    for b15 in range(0x100):
        for b16 in range(0x100):
            key = b"\xff" * 13 + bytes([b14, b15, b16])
            count += 1
            
            current_time = time.time()
            if current_time - last_update >= 1.0:
                elapsed = current_time - start_time
                progress = (count / total_keys) * 100
                keys_per_sec = count / elapsed if elapsed > 0 else 0
                eta_seconds = (total_keys - count) / keys_per_sec if keys_per_sec > 0 else 0
                
                print(f"\r[Progress] {progress:5.2f}% | "
                      f"{count:,}/{total_keys:,} keys | "
                      f"Speed: {keys_per_sec:,.0f} keys/s | "
                      f"ETA: {eta_seconds:.0f}s | "
                      f"Candidates: {len(found_candidates)}", end="", flush=True)
                ...
                if plaintext[:3] == b'DH{':
                    ...
                    try:
                        ...
                        if all(32 <= ord(c) <= 126 for c in text):
                            found_candidates.append((key, plaintext, text))
                            print(f"\n\n{'='*70}")
...
if found_candidates:
    print("\nAll candidates:")
    for i, (key, plaintext, text) in enumerate(found_candidates, 1):
        print(f"{i}. Key: {key.hex()}")
