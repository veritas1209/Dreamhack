...
extern const short _m;
extern const short blk;
extern const short r;
	...
	unsigned char *c, const unsigned char *m, const unsigned char *pk)
{
	...
	for( i=0 ; i<n ; i++ )
	{
		randombytes((unsigned char *)&lin,4);
		lin &= flag;
...
void encrypt_one_block_INDCCA2(unsigned char *c, const unsigned char *m, const unsigned char *pk)
{
	...
	randombytes_init(rand_seed,NULL,48);
	rand_value = (unsigned char *)malloc((size_t)msg_len+CONST_LEN);
	randombytes(rand_value,msg_len+CONST_LEN);
	...
	const unsigned char *pk)
{
	...
	if( real_mlen > mlen+4 )
		randombytes(msg + 4 + mlen, real_mlen - mlen - 4);
