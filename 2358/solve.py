import itertools

v20 = 100

# S-box
v26 = []

# 역 S-box
inv_v26 = [0] * 256
for i in range(256):
    inv_v26[v26[i]] = i

def v8(text):
    return [list(text[i:i+2]) for i in range(0, len(text), 2)]

def v9(v17):
    return bytes(sum(v17, []))

def v12(v13, v14=4, v15=True):
    return [v13[i:i+4] for i in range(0, len(v13), v14)]

def v6(k):
    for i in range(4):
        for j in range(2*v20):
            k[i] = pow(pow(v26[v26[v26[((((k[i] << 4) ^ k[i]) << 4) ^ k[i]) % 256]]], pow(k[i], k[i]), 256), pow(v26[k[i]], v26[k[i]]), 256)

def v7(k):
    for i in range(4):
        k[i] = v26[k[i]]

def inv_v7(k):
    for i in range(4):
        k[i] = inv_v26[k[i]]

def inv_v5(s, k):
    for i in range(2):
        s[i][1] ^= k[k[k[k[i*2+1]%4]%4]%4]

def inv_v2(s):
    s[0][1], s[1][1] = s[1][1], s[0][1]

def inv_v22(s):
    for i in range(2):
        s[i][1] = inv_v26[s[i][1]]

def inv_v4(s, k):
    for i in range(2):
        for j in range(2):
            s[i][j] ^= k[k[k[k[i*2+j]%4]%4]%4]

def inv_v3s(state):
    for i in range(2):
        v25 = []
        for j in range(2): v25.append(state[i][j])
        v25[0] = v25[0] ^ v25[1]
        for j in range(2): state[i][j] = v25[j]

def inv_v1(s):
    s[0][0], s[1][0] = s[1][0], s[0][0]

def inv_v23(s):
    for i in range(2):
        s[i][0] = inv_v26[s[i][0]]

def generate_round_keys(k):
    round_keys = []
    key = k.copy()
    round_keys.append(key.copy())
    
    for j in range(v20):
        v6(key)
        round_keys.append(key.copy())
    
    v7(key)
    round_keys.append(key.copy())
    
    return round_keys

def decrypt_block(ciphertext_block, k):
    round_keys = generate_round_keys(k)
    s = v8(ciphertext_block)
    
    inv_v5(s, round_keys[-1])
    inv_v2(s)
    inv_v22(s)
    inv_v7(round_keys[-1])
    
    for j in range(v20-1, -1, -1):
        inv_v4(s, round_keys[j+1])
        inv_v3s(s)
        inv_v1(s)
        inv_v23(s)
    
    inv_v4(s, round_keys[0])
    
    return v9(s)

def brute_force_key(ciphertext_hex):
    ciphertext = bytes.fromhex(ciphertext_hex)
    blocks = v12(ciphertext)
    
    print(f"Searching for key with DH{{ pattern...")
    
    for a, b, c, d in itertools.product(range(256), repeat=4):
        if a % 10 == 0 and b == 0 and c == 0 and d == 0:
            print(f"Progress: [{a}, 0, 0, 0]")
        
        key = [a, b, c, d]
        
        try:
            plaintext = decrypt_block(blocks[0], key)
            
            if plaintext.startswith(b'DH{'):
                print(f"\n*** KEY FOUND: {key} ***")
                
                full_plaintext = b""
                for block in blocks:
                    full_plaintext += decrypt_block(block, key)
                
                try:
                    pad_len = full_plaintext[-1]
                    if 1 <= pad_len <= 4:
                        full_plaintext = full_plaintext[:-pad_len]
                except:
                    pass
                
                return key, full_plaintext
        except:
            continue
    
    return None, None

# 실행
ciphertext_hex = "34a0fb58f3f5e740bc663d2e01c9d320842e5af5b38c4e7b08f9029825f50540fd8cee2019f5d7e6a89c6164a6911c08"
key, plaintext = brute_force_key(ciphertext_hex)

if key:
    print(f"Key: {key}")
    print(f"Flag: {plaintext.decode('utf-8', errors='ignore')}")
else:
    print("Key not found")