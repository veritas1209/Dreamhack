...
static uint16_t mtt_mldsa_last_attempts_used = 0;
static uint8_t mtt_mldsa_last_profile_lanes[(MLDSA_K * MLDSA_N) / 2];
static uint8_t mtt_mldsa_requested_profile_lane = 0;
enum
{
  MTT_MLDSA_REBALANCE_STRIDE = 32
};
...
int mtt_mldsa_profile_last_attempts(uint16_t *out)
{
  ...
  *out = mtt_mldsa_last_attempts_used;
  return 0;
}
...
int mtt_mldsa_profile_select_lane(uint8_t lane)
{
  ...
  mtt_mldsa_requested_profile_lane = lane;
  return 0;
}
...
{
  size_t i;
  const size_t lane_len = MLDSA_N / 2;
  const size_t offset = (size_t)lane * lane_len;

  if (lane >= MLDSA_K || out == NULL || outlen == NULL || *outlen < lane_len)
  {
    return -1;
  }
  ...
  {
    out[i] = mtt_mldsa_last_profile_lanes[offset + i];
  }
  *outlen = lane_len;
  return 0;
}

static void mtt_mldsa_profile_reset(void)
{
  mtt_mldsa_last_attempts_used = 0;
  mld_memset(mtt_mldsa_last_profile_lanes, 0, sizeof(mtt_mldsa_last_profile_lanes));
}

static void mtt_mldsa_pack_poly_mod16(uint8_t *dst, const mld_poly *poly)
{
  size_t coeff_idx;

  for (coeff_idx = 0; coeff_idx < MLDSA_N; coeff_idx += 2)
  {
    const uint8_t lo = (uint8_t)((uint32_t)poly->coeffs[coeff_idx] & 0x0fu);
    const uint8_t hi =
        (uint8_t)((uint32_t)poly->coeffs[coeff_idx + 1] & 0x0fu);
    dst[coeff_idx / 2] = (uint8_t)(lo | (uint8_t)(hi << 4));
  }
}

static void mtt_mldsa_apply_public_rebalance(
    mld_polyvecl *y, const uint8_t profile_seed[MLDSA_CRHBYTES], uint16_t nonce)
{
  static const uint8_t domain[] = {'m', 't', 't', ':', 'y', '3'};
  uint8_t rebalance[MLDSA_N + 1];
  uint8_t nonce_bytes[2];
  mld_shake256ctx state;
  const size_t profile_lane = (size_t)mtt_mldsa_requested_profile_lane;
  size_t coeff_bucket;
  size_t coeff_idx;

  nonce_bytes[0] = (uint8_t)(nonce & 0xffu);
  nonce_bytes[1] = (uint8_t)(nonce >> 8);

  mld_shake256_init(&state);
  mld_shake256_absorb(&state, profile_seed, MLDSA_CRHBYTES);
  mld_shake256_absorb(&state, nonce_bytes, sizeof(nonce_bytes));
  mld_shake256_absorb(&state, domain, sizeof(domain));
  mld_shake256_finalize(&state);
  mld_shake256_squeeze(rebalance, sizeof(rebalance), &state);
  mld_shake256_release(&state);
  coeff_bucket = (size_t)(rebalance[0] & (MTT_MLDSA_REBALANCE_STRIDE - 1u));

  for (coeff_idx = coeff_bucket; coeff_idx < MLDSA_N;
       coeff_idx += MTT_MLDSA_REBALANCE_STRIDE)
  {
    const int32_t target = (int32_t)(rebalance[coeff_idx + 1] & 0x07u);
    int32_t coeff = y->vec[profile_lane].coeffs[coeff_idx];
    int32_t current = coeff % 8;
    int32_t delta;
    ...
    coeff += delta;
    if (coeff > MLDSA_GAMMA1)
    {
      coeff -= 8;
    }
    else if (coeff < -(MLDSA_GAMMA1 - 1))
    {
      coeff += 8;
    }
    y->vec[profile_lane].coeffs[coeff_idx] = coeff;
  }
}
...
static int mld_attempt_signature_generation(
    ...
    const uint8_t profile_seed[MLDSA_CRHBYTES], uint16_t nonce, mld_polymat *mat,
...
)
{
  ...
  int ret;
  uint8_t profile_lanes[(MLDSA_K * MLDSA_N) / 2];
  ...
  mtt_mldsa_apply_public_rebalance(y, profile_seed, nonce);
  ...
  *tmp = *y;
  ...
  )
  {
    ...
    *t = w0->vec[k];
    ...
    mld_poly_invntt_tomont(z);
    mld_poly_reduce(z);
    mtt_mldsa_pack_poly_mod16(profile_lanes + (k * (MLDSA_N / 2)), z);
  ...
  mld_memcpy(mtt_mldsa_last_profile_lanes, profile_lanes, sizeof(profile_lanes));
  ret = 0; /* success */
                                ...
                                MLD_CONFIG_CONTEXT_PARAMETER_TYPE context)
{
  ...
  uint8_t *profile_seed;
  uint16_t nonce = 0;
  MLD_ALLOC(seedbuf, uint8_t,
            2 * MLDSA_SEEDBYTES + MLDSA_TRBYTES + 3 * MLDSA_CRHBYTES, context);
  MLD_ALLOC(mat, mld_polymat, 1, context);
  ...
  profile_seed = rhoprime + MLDSA_CRHBYTES;
  mld_unpack_sk(rho, tr, key, t0hat, s1hat, s2hat, sk);
  mtt_mldsa_profile_reset();
  ...
  mld_H(rhoprime, MLDSA_CRHBYTES, key, MLDSA_SEEDBYTES, rnd, MLDSA_RNDBYTES, mu,
        MLDSA_CRHBYTES);
  /* Public profile seed: operationally intended for queue balancing.
   * Because it is derived from tr instead of key, the low-bit steering
   * becomes reproducible from public data once rnd is known. */
  mld_H(profile_seed, MLDSA_CRHBYTES, tr, MLDSA_TRBYTES, rnd, MLDSA_RNDBYTES,
        mu, MLDSA_CRHBYTES);
  ...
  )
  {
    ...
    ret = mld_attempt_signature_generation(sig, mu, rhoprime, profile_seed,
                                           nonce, mat, s1hat, s2hat, t0hat,
                                           context);
    nonce++;
    if (ret == 0)
    {
      mtt_mldsa_last_attempts_used = nonce;
