...
/* 무조건 리틀 엔디안(x86_64 리눅스 환경)으로 강제 고정 */
...
int EncKeySetup(const Byte *mk, Byte *rk, int keyBits) {
  ...
  GSRK(w0, w1, 19); GSRK(w1, w2, 19); GSRK(w2, w3, 19); GSRK(w3, w0, 19);
  GSRK(w0, w1, 31); GSRK(w1, w2, 31); GSRK(w2, w3, 31); GSRK(w3, w0, 31);
  GSRK(w0, w1, 67); GSRK(w1, w2, 67); GSRK(w2, w3, 67); GSRK(w3, w0, 67);
  GSRK(w0, w1, 97);
  if (keyBits > 128) { GSRK(w1, w2, 97); GSRK(w2, w3, 97); }
  if (keyBits > 192) { GSRK(w3, w0,  97); GSRK(w0, w1, 109); }
...
// ---------------------------------------------------------
// 회원님의 CTF를 위한 맞춤형 main 함수 (ARIA 복호화기)
// ---------------------------------------------------------
// ---------------------------------------------------------
// 궁극의 ARIA 복호화기 (모든 단서 자동 스캔)
// ---------------------------------------------------------
int main(int argc, char **argv) {
    if (argc != 2) {
        printf("Usage: %s <encrypted_file.enc>\n", argv[0]);
        return 1;
    }
    ...
    Byte *file_data = (Byte *)malloc(fsize);
    fread(file_data, 1, fsize, fin);
    fclose(fin);

    // [단서 모음]
    Byte hex_cc[16] = {0xcc, 0x7f, 0x68, 0x87, 0x34, 0xe7, 0x70, 0xde, 0xc3, 0x14, 0x01, 0x42, 0xff, 0xae, 0x82, 0xab};
    Byte hex_aa[16] = {0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa};
    Byte str_cc[32] = "cc7f688734e770dec3140142ffae82ab";
    Byte str_aa[32] = "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
    Byte str_a16[16] = "AAAAAAAAAAAAAAAA"; // 0x41 16개
    // OCaml 메모리에서 뜯어냈던 역계산 배열
    Byte ocaml_1[16] = {0xb7, 0x72, 0x69, 0xd4, 0x0c, 0xfe, 0x38, 0x1a, 0x65, 0xd2, 0x9c, 0xb4, 0xa8, 0x1f, 0x43, 0xee};
    Byte ocaml_2[16] = {0x4d, 0x0f, 0x1c, 0x2a, 0x84, 0xb7, 0x96, 0xe3, 0x5f, 0x1a, 0x20, 0xc8, 0xde, 0x9b, 0x73, 0xa1};

    // 키 후보군 7개
    Byte keys[7][32];
    int key_sizes[7] = {128, 128, 128, 256, 256, 128, 128};
    memcpy(keys[0], hex_cc, 16);
    memcpy(keys[1], hex_aa, 16);
    memcpy(keys[2], str_a16, 16);
    memcpy(keys[3], str_cc, 32);
    memcpy(keys[4], str_aa, 32);
    memcpy(keys[5], ocaml_1, 16);
    memcpy(keys[6], ocaml_2, 16);

    // IV 후보군 5개
    Byte ivs[5][16];
    memcpy(ivs[0], hex_cc, 16);
    memcpy(ivs[1], hex_aa, 16);
    memcpy(ivs[2], str_a16, 16);
    memset(ivs[3], 0, 16); // All Zero
    memcpy(ivs[4], ocaml_1, 16);

    Byte *plain = (Byte *)malloc(fsize);

    printf("[*] Starting Universal ARIA Brute-Force...\n");

    // 경우의 수 1: 암호화 모드 (CBC / ECB)
    for(int is_cbc = 1; is_cbc >= 0; is_cbc--) {
        // 경우의 수 2: IV 위치 (파일 맨 앞 / 하드코딩)
        for(int is_prepended = 0; is_prepended <= 1; is_prepended++) {
            long csize = is_prepended ? fsize - 16 : fsize;
            Byte *cipher = is_prepended ? file_data + 16 : file_data;

            // 경우의 수 3: 마스터 키 종류 (7가지)
            for(int k = 0; k < 7; k++) {
                // 경우의 수 4: IV 종류 (5가지)
                for(int v = 0; v < 5; v++) {
                    Byte iv[16];
                    ...
                    Byte rk[16*17];
                    DecKeySetup(keys[k], rk, key_sizes[k]);

                    Byte prev_block[16];
                    memcpy(prev_block, iv, 16);
                    int rounds = (key_sizes[k] == 128) ? 12 : 16;

                    // 첫 번째 블록만 먼저 복호화 해보기
                    Byte dec_block[16];
                    Crypt(cipher, rounds, rk, dec_block);
                    
                    if (is_cbc) {
                        for (int j = 0; j < 16; j++) plain[j] = dec_block[j] ^ prev_block[j];
                    } else {
                        for (int j = 0; j < 16; j++) plain[j] = dec_block[j];
                    }

                    // 드디어! PNG 헤더 매칭 여부 확인 (89 50 4E 47)
                    if (plain[0] == 0x89 && plain[1] == 0x50 && plain[2] == 0x4E && plain[3] == 0x47) {
                        printf("\n[🔥 BINGO!!! 엑조디아 완성 🔥]\n");
                        printf("- Mode: %s\n", is_cbc ? "CBC" : "ECB");
                        printf("- IV Location: %s\n", is_prepended ? "File Prepended" : "Hardcoded in Binary");
                        printf("- Key Size: ARIA-%d\n", key_sizes[k]);
                        
                        // PNG 헤더를 찾았다면 전체 파일 복호화 진행!
                        memcpy(prev_block, iv, 16);
                        for (long i = 0; i < csize; i += 16) {
                            Crypt(cipher + i, rounds, rk, dec_block);
                            if (is_cbc) {
                                for (int j = 0; j < 16; j++) plain[i+j] = dec_block[j] ^ prev_block[j];
                            } else {
                                for (int j = 0; j < 16; j++) plain[i+j] = dec_block[j];
                            }
                            memcpy(prev_block, cipher + i, 16);
                        }
                        
                        char out_name[256];
                        sprintf(out_name, "%s.dec", argv[1]);
                        FILE *fout = fopen(out_name, "wb");
                        fwrite(plain, 1, csize, fout);
                        fclose(fout);
                        
                        printf("- Decrypted file saved to: %s\n", out_name);
                        return 0; // 찾았으면 프로그램 즉시 종료
                    }
                }
                if (is_prepended) break; // 파일 앞단 IV를 쓰면 하드코딩 IV 루프는 무의미함
            }
        }
    }

    printf("[-] Failed... The header '89 50 4E 47' was not found.\n");
    return 0;
}